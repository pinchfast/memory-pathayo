...
from kgmemory.db.config import TORTOISE_ORM
from kgmemory.users import utils
from kgmemory.users.schemas import UserCreate
...
def work(mailserver: bool = typer.Option(False)):
    ...
    manager.add_process(
        "falkordb",
        "docker run --rm -p 6380:6379 falkordb/falkordb:latest "
        "|| echo 'falkordb already running on 6380'",
    )
    manager.add_process("server", f"aerich upgrade && {sys.executable} -m kgmemory dev", env=project_env)
    manager.add_process("worker", f"{sys.executable} -m kgmemory run-worker", env=project_env)
    if mailserver:
        manager.add_process("mailserver", f"{sys.executable} -m kgmemory run-mailserver")
...
def run_prod_server():
    ...
    class APPServer(Application):
        ...
        def load(self):
            return util.import_app("kgmemory.main:app")
...
def start_app(app_name: str):
    ...
    files = {
        "__init__.py": "",
        "models.py": "from kgmemory.db.models import TimeStampedModel",
...
def run_worker(reload: bool = typer.Option(True)):
    ...
    if reload:
        subprocess.run(["hupper", "-m", "saq", "kgmemory.worker.settings", "--web"])
    else:
        subprocess.run(["python", "-m", "saq", "kgmemory.worker.settings", "--web"])
