...
from knowledgegraphforpinchfast.db.config import TORTOISE_ORM
from knowledgegraphforpinchfast.users import utils
from knowledgegraphforpinchfast.users.schemas import UserCreate
...
def work(mailserver: bool = typer.Option(False)):
    ...
    manager.add_process("server", f"aerich upgrade && {sys.executable} -m knowledgegraph for pinchfast dev", env=project_env)
    manager.add_process("worker", f"{sys.executable} -m knowledgegraph for pinchfast run-worker", env=project_env)
    if mailserver:
        manager.add_process("mailserver", f"{sys.executable} -m knowledgegraph for pinchfast run-mailserver")
...
def run_server(
    ...
    migrate_db()
    uvicorn.run(
        "knowledgegraph for pinchfast.main:app",
        host=host,
        port=port,
        log_level=log_level,
        reload=reload,
...
def run_prod_server():
    ...
    class APPServer(Application):
        ...
        def load(self):
            return util.import_app("knowledgegraph for pinchfast.main:app")
...
def start_app(app_name: str):
    ...
    files = {
        "__init__.py": "",
        "models.py": "from knowledgegraph for pinchfast.db.models import TimeStampedModel",
...
def run_worker(reload: bool = typer.Option(True)):
    ...
    if reload:
        subprocess.run(["hupper", "-m", "saq", "knowledgegraph for pinchfast.worker.settings", "--web"])
    else:
        subprocess.run(["python", "-m", "saq", "knowledgegraph for pinchfast.worker.settings", "--web"])
